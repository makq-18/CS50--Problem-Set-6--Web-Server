...
char* indexes(const char* path)
{
    //CREAT:varible length for length of string path
    int length = strlen(path);
    
    //CREATE:varibale to be use for allocating memories
    int size = sizeof(char);
    
    //CREATE:if path is for php or html
    char* path_php = malloc((length + 11) * size);
    
    char* path_html = malloc((length + 12) * size);
    
    //CHECK: if memory is allocated for php_path and html_path
    if (path_php == NULL)
    {
        return NULL;
    }
    ...
    //CONCATENATE: if path is php or html
    strcpy(path_php, path);
    strcat(path_php, "/index.php");
    
    strcpy(path_html, path);
    strcat(path_html, "/index.html");
    
    //CHECK: if path is php or html
    if (access(path_php, F_OK) == -1)
    {
        free(path_php);
    }
    else 
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // varibales buffer, track and load to read and maintian track of file memory
    char* buffer = NULL;
    ...
    {
        // if capacity exceeds, then realloc memory by twice
        if (track + 1 > load)
        {
            if (load == 0)
                load = 2;
            ...
            char* temp = realloc(buffer, (load * sizeof(char)));
            if (temp == NULL)
            {
                free(buffer);
                return false;
    ...
    // copy the contents of buffer into string varibale storage and release its memory.
    char* storage = malloc((track) * sizeof(char));
    memcpy(storage, buffer, track);
    free(buffer);
    
    
    //assign the address of contents to double pointer content and count of contents to double pointer length.
    *content = storage;
    *length = track;
    return true;
}
...
const char* lookup(const char* path)
{
    char *needle[] = {".css",".html",".gif",".ico",".jpg",".js",".php",".png"};
    //copy the string 'after' . into string variable check
    ...
    // iterate over each extension
        for(int i=0; i<8; i++)
        {
            if(strcasecmp(check, needle[i]) == 0)
            {
                switch(i)
                {
                    ...
                    case 6: return "text/x-php";
                    case 7: return "image/png";
                    default: return NULL;
                }
            }
        }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // calculate the length of line string and add 1 for null terminator
    int size = ((length+1) * sizeof(char));
    
    //create a temporary copy of line so we can extract various parts
    //copy line into temp_line variable.
    char temp_line[size];
    strcpy(temp_line, line);
    
    // string varibales method, request-line and http version
    char method[size];
    char request[size];
    char version[size];
    
    //extract first part from the temp_line
    char* extract = strtok(temp_line, " ");
    
    //init a token count to 0
    int count = 0;
    
    //get remaining tokens and store in method, target and version
    while (extract != NULL)
    {
        if (count == 0)
        {
            strcpy(method, extract);
        }
        else if (count == 1)
        {
            strcpy(request, extract);
        }
        else if (count == 2)
        {
            strcpy(version, extract);
        }
        else
        {
    ...
    //if request line contains more than three arguments or segments.
    if(count != 3)
    {
        // 400 bad request
        error(400);
    ...
    //if request-line contains any query ? , memory to which query points will be at least of length LimitRequestLine + 1.
    if (strchr(request, '?') == NULL)
    {
    ...
    // if line contains query ?, then extract the query into queryExtract variable
    else if(strchr(request, '?') != NULL)
    {
        count = 0;
        char* queryExtract = strtok(request, "?");
        
        while (queryExtract != NULL)
        {
            if (count == 0)
            {
                strcpy(abs_path, queryExtract);
            }
            else if(count == 1)
            {
                strcpy(query, queryExtract);
            }
            queryExtract = strtok(NULL, "?");
            count++;
        }
    }
    return true;
